...
template <typename T>
ffi::Error GatherImpl(MPI_Comm comm, int root, int numel, int sendcount,
...
ffi::Error GatherDispatch(int64_t comm_handle, int64_t root, int64_t sendcount, ffi::AnyBuffer x,
                          ffi::AnyBuffer token,
                          ffi::Result<ffi::AnyBuffer> y,
                          ffi::Result<ffi::AnyBuffer> token_out)
{
  ...
  ELEMENT_TYPE_DISPATCH(dtype, GatherImpl, comm, root_int, numel_int, sendcount_int, x, token, y, token_out);
}
#endif // GATHER_H
